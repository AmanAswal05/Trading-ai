...
import { detectRegime, MarketRegime } from './regime-detector';
import { applyRegimeMultipliers } from './regime-configs';
import { createSnapshot, findSimilarSetups } from './similarity-engine';
import { buildCalibrationCurve, calibrateConfidence } from './calibration-engine';
...
export function generatePrediction(
  ...
  const regimeClass = detectRegime(indicators, price);
  ...
  const baseConfidence = Math.min(100, Math.max(0, Math.round((weightedScore / totalContribution) * 100)));
  ...
  let insufficientEdge = false;

  if (typeof window === 'undefined') {
    try {
      const { PredictionsDbService } = require('./predictions-db');
      const allPredictions = PredictionsDbService.getAllPredictions ? PredictionsDbService.getAllPredictions() : [];
      // Resolve promise synchronously if mock db, or if it returns resolved array
      const resolvedPredictions = allPredictions && typeof allPredictions.then === 'function' 
        ? [] // Fallback for async
        : (allPredictions || []);
      ...
      if (resolvedPredictions.length > 0) {
        ...
        // Apply global confidence calibration curve
        ...
        // Phase 9: Stock Eligibility Filter
        const tickerRecords = resolvedPredictions.filter((p: any) => p.ticker === ticker && p.status === 'VERIFIED');
        if (tickerRecords.length >= 15) {
          const correct = tickerRecords.filter((r: any) => r.prediction_result === 'CORRECT').length;
          const partial = tickerRecords.filter((r: any) => r.prediction_result === 'PARTIALLY_CORRECT').length;
          const total = tickerRecords.length;
          const tickerAccuracy = ((correct + partial * 0.5) / total) * 100;

          if (tickerAccuracy < 45.0) {
            // Poor predictive performance ticker. Apply confidence penalty.
            calibratedConfidence = Math.round(calibratedConfidence * 0.82);
            if (calibratedConfidence < 58) {
  ...
  let signalStrength: 'NO_SIGNAL' | 'WEAK_SIGNAL' | 'STRONG_SIGNAL' | 'HIGH_CONVICTION' = 'NO_SIGNAL';
  if (confidence >= 85) signalStrength = 'HIGH_CONVICTION';
  else if (confidence >= 70) signalStrength = 'STRONG_SIGNAL';
  else if (confidence >= 55) signalStrength = 'WEAK_SIGNAL';

  // Determine direction
  let direction: 'UP' | 'DOWN' | 'NEUTRAL' = 'NEUTRAL';
  if (confidence >= config.upThreshold) {
    direction = ensemble.finalDirection === 'UP' ? 'UP' : 'NEUTRAL';
  } else if (confidence <= config.downThreshold) {
    direction = ensemble.finalDirection === 'DOWN' ? 'DOWN' : 'NEUTRAL';
  }

  // If no signal or insufficient edge, override direction to NEUTRAL
  if (signalStrength === 'NO_SIGNAL' || insufficientEdge) {
    direction = 'NEUTRAL';
  }
  ...
  let bullishProb = ensemble.combined.bullish;
  let bearishProb = ensemble.combined.bearish;
  let neutralProb = ensemble.combined.neutral;
